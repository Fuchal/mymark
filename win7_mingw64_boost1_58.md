...
###PS：b2和bjam是一样的东西，这里列了一下相关的参数
	...
	--build-type=complete          编译所有版本，不然只会编译一小部分版本（确切地说是相当于:variant=release, threading=multi;link=shared|static;runtime-link=shared）variant=debug|release          决定编译什么版本(Debug or Release?) 
...
###参考：
	...
	例子，参考script目录上的tboost.cpp，然后执行
